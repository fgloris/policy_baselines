...
def _init_vector_env_compat(self, num_envs, observation_space, action_space):
    """Initialize fields expected by old Gym VectorEnv code.

    Gymnasium's VectorEnv.__init__ signature changed across versions and may
    not accept num_envs/observation_space/action_space. This project uses the
    old Gym vector-env API internally, so we set the required attributes here.
    """
    try:
        ...
        try:
            super(type(self), self).__init__(observation_space, action_space, num_envs)
        except TypeError:
            try:
                super(type(self), self).__init__()
            except TypeError:
...
def _read_from_shared_memory_compat(space, shared_memory, n=1):
    try:
...
def _write_to_shared_memory_compat(space, index, value, shared_memory):
    try:
...
def _concatenate_compat(space, items, out):
    try:
...
class AsyncVectorEnv(VectorEnv):
    ...
    def __init__(
        ...
        if self.shared_memory:
            try:
                ...
                self.observations = _read_from_shared_memory_compat(
    ...
    # Old Gym-style API used by the runners in this repository.
    # Gymnasium's VectorEnv.reset/step use a newer return convention, so keep
    # these wrappers local to avoid API translation surprises.
    def reset(self):
        self.reset_async()
    ...
    def reset_wait(self, timeout=None):
        ...
        if not self.shared_memory:
            self.observations = _concatenate_compat(
...
def _worker_shared_memory(index, env_fn, pipe, parent_pipe, shared_memory, error_queue):
    ...
    try:
        while True:
            ...
            if command == "reset":
                observation = env.reset()
                _write_to_shared_memory_compat(
